...
import java.awt.Dimension; // Keep Dimension data structure
// AWT imports removed - using JavaFX
import java.io.File;
...
public class mudclient extends Shell {
    ...
    public void playSoundEffect(SoundEffect sound) {
        /* Audio disabled: ignore sound effect requests */
    }
    ...
    protected void resetGame() {
        ...
        surface.blackScreen();
        // Direct canvas rendering (JavaFX)
    ...
    @Override
    public javafx.scene.image.Image createImage(int i, int j) {
        if (application != null) {
            return new javafx.scene.image.WritableImage(i, j);
        }
    ...
    public void resetLoginVars() {
        ...
        // application.setResizable() removed (JavaFX Stage)
        // application.setSize() removed (JavaFX Stage)
        // application.setLocationRelativeTo() removed (JavaFX Stage)
    }
...
OUTER:		for (int animationIndex = 0; animationIndex < EntityManager.getAnimations().length; animationIndex++) {
    ...
    protected void startGame() {
        ...
        graphics = getGraphicsContext();
        setTargetFps(50);
        surface = new Screen(gameWidth, gameHeight + 12, 4000);
        surface.mc = this;
        surface.setBounds(0, 0, gameWidth, gameHeight + 12);
    ...
    protected void draw() {
        ...
        try {
            ...
            // Render Surface pixel buffer to JavaFX Canvas
            renderToCanvas();
    ...
    // Reusable WritableImage for rendering to avoid creating new images every frame
    private javafx.scene.image.WritableImage renderImage = null;
    private int[] argbPixelBuffer = null;
    
    private void renderToCanvas() {
        // Initialize reusable buffers on first call
        if (renderImage == null || renderImage.getWidth() != surface.width2 || renderImage.getHeight() != surface.height2) {
            renderImage = new javafx.scene.image.WritableImage(surface.width2, surface.height2);
            argbPixelBuffer = new int[surface.imagePixelArray.length];
        }
        
        // Convert RGB pixel buffer to ARGB for JavaFX
        for (int i = 0; i < surface.imagePixelArray.length; i++) {
            int rgb = surface.imagePixelArray[i];
            // Convert RGB (0x00RRGGBB) to ARGB (0xAARRGGBB) - add full opacity
            argbPixelBuffer[i] = 0xFF000000 | rgb;
        }
        
        // Update the WritableImage with new pixel data (can be done off JavaFX thread)
        try {
            javafx.scene.image.PixelWriter pw = renderImage.getPixelWriter();
            pw.setPixels(0, 0, surface.width2, surface.height2, 
                javafx.scene.image.PixelFormat.getIntArgbInstance(), 
                argbPixelBuffer, 0, surface.width2);
            
            // Draw directly - NO Platform.runLater
            javafx.scene.canvas.GraphicsContext gc = getCanvas().getGraphicsContext2D();
            gc.drawImage(renderImage, 0, 0);
        } catch (Exception e) {
    ...
    private void login(String u) {
        ...
        switch (loginResponse) {
            ...
            case 50:
                resetGame();
                // application.setResizable() removed (JavaFX Stage)
                // removed temp: // application.setSize() removed (JavaFX Stage)
                // application.setLocationRelativeTo() removed (JavaFX Stage)
                break;
            case 1:
