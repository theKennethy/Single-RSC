...
public class PathHandler {
    ...
    protected int[] getNextCoords(int startX, int destX, int startY, int destY) {
        try {
            ...
            //if(!Config.MEMBERS_CONTENT && destY > 95 && destY < 142) {
            //  cancelCoords();
            //}
    ...
    private boolean isMobBlocking(int x, int y) {
        ...
        if (mob instanceof Player) {
            if (t.hasNpcs()) {
                //if (Constants.GameServer.NPCS_BLOCK_PATH) {
                for (NPC n : t.getNpcs()) {
    ...
    protected void setNextPosition() {
        ...
        if (newCoords[0] > -1 && newCoords[1] > -1) {
            if(mob instanceof NPC) {
                NPC n = (NPC) mob;
                n.setLocation(new Point(newCoords[0], newCoords[1]));
            } else if(mob instanceof Player) {
                Player p = (Player) mob;
                p.setLocation(new Point(newCoords[0], newCoords[1]));
            }
        }
    } 
